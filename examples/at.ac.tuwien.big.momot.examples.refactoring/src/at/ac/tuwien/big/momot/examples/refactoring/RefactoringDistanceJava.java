...
public class RefactoringDistanceJava {
   ...
   public static void initialization() {
      ...
      RefactoringValueDistance.initWith(new File(FINAL_MODEL));
      RefactoringMoveDistance.initWith(new File(FINAL_MODEL));
      RefactoringElementDistance.initWith(new File(FINAL_MODEL));
   }

   public static void main(final String... args) {
      final File inputFolder = new File("allinputs");
      final File outputFolder = new File("alloutputs-" + new Date().getTime());
      for(final File input : inputFolder.listFiles()) {
         ...
         initialization();
         final RefactoringDistanceJava search = new RefactoringDistanceJava();
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_1(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("ValueDistance",
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_2(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("MoveDistance",
            at.ac.tuwien.big.moea.search.fitness.dimension.IFitnessDimension.FunctionType.Minimum) {
         @Override
         protected double internalEvaluate(final TransformationSolution solution) {
            final EGraph graph = solution.execute();
   ...
   protected IFitnessDimension<TransformationSolution> _createObjective_3(
         final TransformationSearchOrchestration orchestration) {
      return new AbstractEGraphFitnessDimension("ElementDistance",
            at.ac.tuwien.big.moea.search.fitness.dimension.IFitnessDimension.FunctionType.Minimum) {
         @Override
         protected double internalEvaluate(final TransformationSolution solution) {
            final EGraph graph = solution.execute();
