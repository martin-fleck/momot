package at.ac.tuwien.big.momot.examples.modularization.icmt;
...
public class CRAIndexCalculator {
   static double calculateCohesion(final ClassModel model) {
      ...
      for(final icmt.tool.momot.demo.architecture.Class clazz : model.getClasses()) {
   ...
   static double calculateCoupling(final icmt.tool.momot.demo.architecture.Class classSource, final ClassModel model) {
      double couplingRatio = 0;
      for(final icmt.tool.momot.demo.architecture.Class classTarget : model.getClasses()) {
   ...
   static List<Attribute> getAttributesClass(final icmt.tool.momot.demo.architecture.Class clazz) {
      final List<Attribute> attributes = new ArrayList<Attribute>();
      for(final Feature feature : clazz.getEncapsulates()) {
         if(feature.getClass().getSimpleName().equals("AttributeImpl")) {
   ...
   static List<Method> getMethodsClass(final icmt.tool.momot.demo.architecture.Class clazz) {
      final List<Method> methods = new ArrayList<Method>();
      for(final Feature feature : clazz.getEncapsulates()) {
         if(feature.getClass().getSimpleName().equals("MethodImpl")) {
   ...
   static int mai(final icmt.tool.momot.demo.architecture.Class classSource,
         final icmt.tool.momot.demo.architecture.Class classTarget) {
      int mai = 0;
   ...
   public static void main(final String[] args) {
      registerPackage();
      // MOMoTStandaloneSetup.doSetup();
      for(final File model : new File("output/models").listFiles()) {
         System.out.println("Processing " + model);
         evaluateModel(loadModel(model.getAbsolutePath()));
      }

   }

   static int mmi(final icmt.tool.momot.demo.architecture.Class classSource,
         final icmt.tool.momot.demo.architecture.Class classTarget) {
      int mmi = 0;
