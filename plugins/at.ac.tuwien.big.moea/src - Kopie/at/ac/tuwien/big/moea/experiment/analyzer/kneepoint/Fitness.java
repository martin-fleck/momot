...
public class Fitness {
     ...
     */
    private NondominatedPopulation nondominatedPopulation;
     ...
     */
    public Fitness(NondominatedPopulation nondominatedPopulation) {
    ...
    public void setFitness(Solution solution, Double fitness, String key) {
    	setFitness(solution, fitness);
    	solution.setAttribute(key, fitness);
    }
    
    public void setProperUtility(Solution solution, Double fitness) {
    	solution.setAttribute(KEY_PROPER_UTILITY, fitness);
    }
    
    public Double getProperUtility(Solution solution) {
    ...
    public void properUtility() {
		...
		for (Solution solution : nondominatedPopulation) {
		    setProperUtility(solution, 0.0);
		}
	
		for (int cur = 0; cur < nondominatedPopulation.size(); cur++) {
		    Solution current = nondominatedPopulation.get(cur);
	
		    for (int oth = cur + 1; oth < nondominatedPopulation.size(); oth++) {
			Solution other = nondominatedPopulation.get(oth);
	
			double[] diff = MathUtil.subtract(nondominatedPopulation.get(cur).getObjectives(), nondominatedPopulation.get(oth).getObjectives());
			double max = MathUtil.max(diff);
			double min = MathUtil.min(diff);
    ...
    public void properUtility(double tradeoff) {
    	properUtility();
    	for (Solution sol : nondominatedPopulation) {
    ...
    public void minmaxTradeoffFitness() {
		...
		for (Solution solution : nondominatedPopulation) {
		    ...
		    setFitness(solution, 
		    		MathUtil.max(MathUtil.divide(
		    				MathUtil.subtract(objectives, best), 
		    				MathUtil.subtract(worst, objectives))),
    ...
    private void utilityKneeFitness2D() {
		...
		setFitness(nondominatedPopulation.get(nondominatedPopulation.size() - 1), Double.MAX_VALUE, KEY_UTILITY_KNEE_FITNESS_2D);
	
		for (int i = 1; i < nondominatedPopulation.size() - 1; i++) {
		    Solution current = nondominatedPopulation.get(i);
    ...
    public void utilityKneeFitnessHigherDimensions(double lambda[][]) {
		...
		for (int i = 0; i < lambda.length; i++) {
		    double[] fitness = new double[nondominatedPopulation.size()];
		    double minFitness = Double.MAX_VALUE;
		    int indexMinFitness = 0;
		    for (int j = 0; j < nondominatedPopulation.size(); j++) {
			fitness[j] = 0.0;
			for (int k = 0; k < lambda[i].length; k++) {
			    fitness[j] += nondominatedPopulation.get(j).getObjective(k) * lambda[i][k];
			}
		    ...
		    Arrays.sort(fitness);
		    Solution update = nondominatedPopulation.get(indexMinFitness);
    ...
    private double getWeightForEqualUtility(int i, int j) {
		Solution sol1 = nondominatedPopulation.get(i);
		Solution sol2 = nondominatedPopulation.get(j);
    ...
    private double marginalUtilityKneeIntegralFunction(int i, double alpha) {
		double f1x = nondominatedPopulation.get(i).getObjective(0);
		double f2x = nondominatedPopulation.get(i).getObjective(1);
		double f1y = nondominatedPopulation.get(i - 1).getObjective(0);
		double f2y = nondominatedPopulation.get(i - 1).getObjective(1);
    ...
    public void bendAngleFitness() {
		...
		int size = nondominatedPopulation.size();
		...
		alpha[0] = 270.0 - getRightBendAngle(nondominatedPopulation.get(0), nondominatedPopulation.get(1));
		beta[0] = alpha[0];
		gamma[0] = 270.0 - getRightBendAngle(nondominatedPopulation.get(0), nondominatedPopulation.get(2));
		delta[0] = gamma[0];
		setFitness(nondominatedPopulation.get(0), Math.max(alpha[0], gamma[0]),
		...
		delta[1] = gamma[1];
		setFitness(nondominatedPopulation.get(1), Math.max(alpha[1], gamma[1]),
				KEY_BEND_ANGLE_FITNESS);
	
		alpha[size - 2] = bendAngle(nondominatedPopulation.get(size - 3), nondominatedPopulation.get(size - 2), nondominatedPopulation.get(size - 1));
		beta[size - 2] = bendAngle(nondominatedPopulation.get(size - 4), nondominatedPopulation.get(size - 2), nondominatedPopulation.get(size - 1));
		...
		alpha[size - 1] = 270.0 - getLeftBendAngle(nondominatedPopulation.get(size - 1), nondominatedPopulation.get(size - 2));
		beta[size - 1] = 270.0 - getLeftBendAngle(nondominatedPopulation.get(size - 1), nondominatedPopulation.get(size - 3));
		...
		for (int i = 2; i < size - 2; i++) {
		    alpha[i] = bendAngle(nondominatedPopulation.get(i - 1), nondominatedPopulation.get(i), nondominatedPopulation.get(i + 1));
		    beta[i] = bendAngle(nondominatedPopulation.get(i - 2), nondominatedPopulation.get(i), nondominatedPopulation.get(i + 1));
		    gamma[i] = bendAngle(nondominatedPopulation.get(i - 1), nondominatedPopulation.get(i), nondominatedPopulation.get(i + 2));
		    delta[i] = bendAngle(nondominatedPopulation.get(i - 2), nondominatedPopulation.get(i), nondominatedPopulation.get(i + 2));
    ...
    public void objectiveSumFitness() {
		for (Solution s : nondominatedPopulation) {
			double value = 0.0;
     ...
     */
    public double[][] writeObjectivesToMatrix(NondominatedPopulation population) {
    ...
    public void bulgeFitness(double[] minima, double[] maxima) {
		for(int curSol = 0; curSol < nondominatedPopulation.size(); curSol++) {
		    double sum = 0.0;
		    for (int obj = 0; obj < nObj; obj++) {
		    	if(Math.abs(minima[obj] - maxima[obj]) <  10E-6) 
		    		sum += (nondominatedPopulation.get(curSol).getObjective(obj) - minima[obj]) / (maxima[obj] - minima[obj]);
		    }
    ...
    public Solution[] obtainCHIM() {
		...
		for (int obj = 0; obj < nObj; obj++) {
			Solution bestKnown = nondominatedPopulation.get(0), candidateSolution;
			int flag;
     ...
     */
    public void convergenceDiversityRanking(Comparator<Solution> convergence, Comparator<Solution> diversity, double c, double d) {
		...
		for (int rank = 0; rank < nondominatedPopulation.size(); rank++) {
			nondominatedPopulation.get(rank).setAttribute(
					"momot_convergence_distance_rank",
     ...
     *            criterion
     */
    public void convergenceDiversityRanking(Comparator<Solution> convergence, Comparator<Solution> diversity) {
    ...
    public double[] writeFitnessToArray(NondominatedPopulation population) {
    ...
    public static void assignFitness(NondominatedPopulation population) {
    	Fitness fitness = new Fitness(population);
    ...
    public static void assignFitness(NondominatedPopulation population, double[] weights) {
    ...
    public static void assignFitness(NondominatedPopulation population, double[] weights, double[] referencePoint) {
