...
public class LinearSingleObjectivizer implements SingleObjectivizer {
	...
	public LinearSingleObjectivizer(double... multiplicateWith) {
		this.values = Arrays.copyOf(multiplicateWith,multiplicateWith.length);
		double divBy = 0.0;
		for (int i = 0; i < values.length; ++i) {
			divBy+= values[i]*values[i];
		}
		if (divBy > 0.0) {
			divBy = 1.0/Math.sqrt(divBy);
			for (int i = 0; i < values.length; ++i) {
				values[i]*=divBy;
			}
		}
	...
	public LinearSingleObjectivizer randomPreference() {
		if (MathUtil.randomDouble() < 0.7) {
			//Just a single object
			//Randomly distribute weights
			double[] weight = new double[values.length];
			double rand = MathUtil.randomDouble();
			rand*=rand;
			//Most often take fewer values
			int takeValues = (int)(values.length*rand)+1;
			for (int i = 0; i < takeValues; ++i) {
				weight[i]+= 1.0;
			}
